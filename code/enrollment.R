...
ggplot(train, aes(Births,Enrollment,color=BirthDecade))+geom_point()+xlab("Births (millions)")+ylab("Enrollment (millions)")+labs(color="Birth decade")
...
g <- ggplot(train, aes(Births,Enrollment))+geom_point()+geom_smooth(method="lm",se=F)
g + geom_point(data=predictions,mapping=aes(Births,y=Enrollment),color="red",size=2)+xlab("Births (millions)")+ylab("Enrollment (millions)")
...
ggplot(combined, aes(EnrollmentYear,Enrollment,color=Type))+geom_point()+geom_line()+geom_hline(yintercept=19,linetype="dashed")+xlab("Year")+ylab("Enrollment (millions)")
