...
ggplot( concrete, aes(Ratio,Strength)) + geom_point() + geom_smooth(method="lm", level=0) 
...
ggplot( concrete, aes(Ratio, Residuals)) + geom_point() + geom_hline(yintercept=0, linetype="dashed")
...
concrete <- concrete %>% mutate( Residuals = concrete.fit$residuals )
concrete.rse <- sigma( concrete.fit )
ggplot( concrete, aes(Ratio, Residuals)) + geom_point() + geom_hline(yintercept = concrete.rse*c(-2,0,2), linetype="dashed")
