...
( cohorts <- cohorts %>% mutate( Null.deviance = 2*( Successes*log(Percentage/Pi.null) + Failures*log((1-Percentage)/(1-Pi.null)) )) )
...
( cohorts <- cohorts %>% mutate(Deviance = 2*( Successes*log(Percentage/Pi.hat) + Failures*log((1-Percentage)/(1-Pi.hat)) )) )
